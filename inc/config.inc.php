...
class Config {
	...
	private function __construct() {
		$db['mysql'] = array(
			'host' 		=> 	'localhost',
			'username' 	=> 	'hardrole_website',
			'password' 	=> 	'mamaejalehardu92',
			'dbname' 	=> 	'hardrole_website'
		);
	...
	public static function getPlayerData2($name,$data) {
		$q = self::$g_con->prepare("SELECT `".$data."` FROM `characters` WHERE `Username` = '$name'");
		$q->execute();
		if($q) {
			$udata = $q->fetch();
	...
	public static function getPlayerCharData($name,$data) {
		$q = self::$g_con->prepare("SELECT `".$data."` FROM `characters` WHERE `Character` = '$name'");
		$q->execute();
		if($q) {
			$udata = $q->fetch();
			return $udata[$data];
